...
const DNAShape = ({ isGenerating }: { isGenerating: boolean }) => {
  ...
  useFrame((state: any, delta: number) => {
    if (groupRef.current) {
      // Unfurl scaling animation
      groupRef.current.scale.lerp(new THREE.Vector3(1, 1, 1), 0.05);
      
      // Speed changes with generation state
      const speed = isGenerating ? 2.5 : 0.5;
      groupRef.current.rotation.y += delta * speed;
    }

    // Energy Pulse Animation (repeats every 2 seconds: state.clock.elapsedTime * Math.PI)
    const pulse = Math.sin(state.clock.elapsedTime * Math.PI); 
    const glowIntensity = 0.2 + (pulse * 0.05); // Oscillates naturally around 0.2
...
const ParticleField = () => {
  ...
  useFrame((state: any) => {
    ...
    particles.forEach((particle, i) => {
      ...
      // Follow mouse with extra kinetic depth
...
export default function DNAViewer({ isGenerating = false }: { isGenerating?: boolean }) {
  // Capping framerate via WebGL params and Frameloop is possible, but hardware-dependent. 
  // We use dpr=[1,1] and disable antialias to save GPU cycles while Bloom is active.
  return (
